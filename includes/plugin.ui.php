...
<!--
   ...
   MyTieIn Infusionsoft Social Login Plugin ( <?php echo $_SESSION["wsl::plugin"] ?> )
   http://wordpress.org/extend/plugins/mytiein-infusionsoft-social-login/
-->
...
<?php 
	...
	foreach( $WORDPRESS_SOCIAL_LOGIN_PROVIDERS_CONFIG AS $item ){
		if($item["provider_id"] != 'Infusionsoft'){
			...
			$assets_base_url = MYTIEIN_INFUS_SOCIAL_LOGIN_PLUGIN_URL . '/assets/img/32x32/' . $social_icon_set; 
	...
	if( $nok ){
		?>
		<p style="background-color: #FFFFE0;border:1px solid #E6DB55;padding:5px;">No provider registered!<br />Please visit the <strong>Settings\ MyTieIn Social Login</strong> administration page to configure this plugin.</p>
		<?php
	}

	// provide popup url for hybridauth callback
	?>
		<input id="wsl_popup_base_url" type="hidden" value="<?php echo MYTIEIN_INFUS_SOCIAL_LOGIN_PLUGIN_URL; ?>/authenticate.php?" />
...
add_action( 'mytiein_infusionsoft_social_login', 'wsl_render_login_form_login' );
...
add_shortcode ( 'mytiein_infusionsoft_social_login', 'wsl_shortcode_handler' );
...
function wsl_add_javascripts()
{
	if( ! wp_script_is( 'wsl_js', 'registered' ) ) {
		wp_register_script( "wsl_js", MYTIEIN_INFUS_SOCIAL_LOGIN_PLUGIN_URL . "/assets/js/connect.js" );
	}
...
function wsl_add_stylesheets(){
	if( ! wp_style_is( 'wsl_css', 'registered' ) ) {
		wp_register_style( "wsl_css", MYTIEIN_INFUS_SOCIAL_LOGIN_PLUGIN_URL . "/assets/css/style.css" ); 
	}
...
function wsl_user_custom_avatar($avatar, $id_or_email, $size, $default, $alt) {
	...
	if( get_option ('wsl_settings_users_avatars') && !empty ($avatar)) {
		...
		// Get the thumbnail provided by MyTieIn Infusionsoft Social Login
		if ($user_id) {
