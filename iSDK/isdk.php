...
#########           Updated by Justin Gourley on 12-19-11        #########
#########           Version 1.7.3                                #########
##########################################################################
...
public function cfgCon($connInfo,$dbOn="on") {
  $this->debug = $dbOn;
  ...
  foreach($appLines as $appLine){
    ...
    if ($details['connectionName'][2]=="i") {
      $this->client = new xmlrpc_client("https://" . $details['connectionName'][1] .".infusionsoft.com/api/xmlrpc");
    } else {
        throw new Exception("Invalid configuration for name: \"" . $details['connectionName'][1] . "\"");
    }
 ...
 // $this->client->setDebug(2);
  ###API Key###
//  echo $details[$name][3];
  $this->key = $details['connectionName'][3];
